...
public class RemoveBlockBreakType implements CommandExecutor {

	@Override
	public boolean onCommand(CommandSender sender, Command cmd, String label,
			String[] args) {
		
		if (!cmd.getName().equalsIgnoreCase(Commands.REMOVE_BLOCK_BREAK_TYPE)) return false;
		...
		if (sender instanceof Player) {
			...
			if (player.isOp()) {
				...
				if (args.length < 1) {
					player.sendMessage("Please enter a valid block type");
					return false;
				}
				...
				if (EnumLists.MATERIAL_TYPES.contains(args[0])) {
					...
					if (Announces.BLOCKS.contains(mat)) {
						...
						//Send in minecraft
						player.sendMessage("Stopped following break event for: " + args[0]);
						...
						DiscordHandler.sendMessage(Announces.MINECRAFT_TO_DISCORD_PERFIX + 
								"Stopped following break event for: " + args[0]);
						DiscordHandler.sendMessage(Announces.MINECRAFT_TO_DISCORD_PERFIX + 
								"Currently following: " + Announces.BLOCKS.toString());
						
					} else {
						player.sendMessage("Given block type not in list");
					}
